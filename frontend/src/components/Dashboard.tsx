        ...
        <div className="flex flex-col gap-3 border-b border-panel-border px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="rounded border border-panel-border bg-background px-2 py-0.5 text-[10px] font-medium text-zinc-500">
          ...
          <div className="flex rounded border border-panel-border bg-background p-1">
            <TabButton active={activeTab === "report"} icon={FileText} label={isBroker ? "Viết report" : "Nhận định"} onClick={() => setActiveTab("report")} />
        ...
        <section className="space-y-3">
          <div className="flex flex-col gap-2 rounded border border-panel-border bg-panel px-4 py-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-zinc-500">Sắp xếp các khối số liệu theo workflow theo dõi thị trường của bạn.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <button
                onClick={fetchSnapshot}
                className="inline-flex h-8 items-center gap-2 rounded border border-panel-border bg-background px-3 text-xs font-semibold text-zinc-200 hover:border-primary/50"
              >
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? "bg-primary text-primary-foreground" : "text-zinc-400 hover:bg-muted hover:text-zinc-100"
      )}
    >
      ...
      <div className="mb-3 flex items-start justify-between gap-2">
        <div className="flex min-w-0 items-center gap-2">
          <button
            {...attributes}
            {...listeners}
            className="cursor-grab rounded border border-panel-border bg-background p-1 text-zinc-600 active:cursor-grabbing"
            aria-label="Kéo để sắp xếp tiện ích"
            title="Kéo để sắp xếp tiện ích"
          >
            <GripVertical className="h-3.5 w-3.5" />
          </button>
          <div className="rounded border border-panel-border bg-background p-1.5 text-primary">
...
function SourceBadge({ label, tone = "neutral" }: { label?: string; tone?: "neutral" | "live" | "warning" }) {
  ...
  return (
    <span
      title={label}
      className={cn(
        "inline-flex w-fit rounded border px-1.5 py-0.5 font-compact text-[9px] font-semibold",
        ...
        tone === "neutral" && "border-panel-border bg-background text-zinc-500"
      )}
    >
      {text}
    </span>
...
function VnIndexWidget({ data }: { data: Snapshot["vnindex"] }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className={cn("font-compact text-3xl font-semibold tabular-nums", isUp ? "text-market-up" : "text-market-down")}>
        {formatNumber(data.price, 2)}
      </div>
      <div className={cn("flex items-center gap-1 font-compact text-xs font-semibold tabular-nums", isUp ? "text-market-up" : "text-market-down")}>
...
function LiquidityWidget({ data }: { data: Snapshot["vnindex"] }) {
  ...
  return (
    <div className="space-y-3">
      <SourceBadge label={data.source_label} tone={data.status === "LIVE" ? "live" : "neutral"} />
      <div>
        <div className="text-[10px] font-semibold text-zinc-600">Giá trị giao dịch</div>
        <div className="font-compact text-3xl font-semibold tabular-nums text-zinc-100">{formatNumber(data.total_value, 0)}</div>
        <div className="text-[11px] text-zinc-500">Tỷ VNĐ</div>
      </div>
      <div className="rounded border border-panel-border bg-background p-2">
        <div className="text-[10px] text-zinc-600">Khối lượng</div>
        <div className="font-compact text-xs font-semibold tabular-nums text-zinc-300">{formatNumber(data.total_volume, 0)} CP</div>
      </div>
    </div>
  );
}
...
function BreadthWidget({ data }: { data: Snapshot["vnindex"] }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex h-2 overflow-hidden rounded-full border border-panel-border bg-background">
...
function ForeignNetWidget({ data }: { data?: Snapshot["foreign"] }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className={cn("font-compact text-3xl font-semibold tabular-nums", isBuy ? "text-market-up" : "text-market-down")}>
        {formatSigned(net / 1e9, 0)}
      </div>
      <div className="text-[11px] text-zinc-500">Tỷ VNĐ</div>
      <span className={cn("inline-flex rounded px-2 py-1 text-[11px] font-semibold", isBuy ? "bg-market-up/10 text-market-up" : "bg-market-down/10 text-market-down")}>
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-3">
        {data.slice(0, 12).map((sector) => (
          <div key={sector.sector} className={cn("min-h-16 rounded border p-2", heatClass(sector.avg_change))}>
            <div className="line-clamp-2 text-[11px] font-semibold leading-tight">{sector.sector}</div>
            <div className="mt-2 font-compact text-lg font-semibold tabular-nums">{formatSigned(sector.avg_change, 2)}%</div>
...
function ImpactColumn({ title, rows, maxValue, tone }: { title: string; rows: ImpactRow[]; maxValue: number; tone: "up" | "down" }) {
  return (
    <div className="space-y-1.5">
      <div className={cn("text-[10px] font-semibold", tone === "up" ? "text-market-up" : "text-market-down")}>{title}</div>
      {rows.slice(0, 6).map((item) => (
        <div key={`${title}-${item.symbol}`} className="relative overflow-hidden rounded border border-panel-border bg-background px-2 py-1.5">
          <div
          ...
          <div className="relative flex items-center justify-between gap-3 text-xs">
            <span className="font-compact font-semibold text-zinc-100">{item.symbol}</span>
            <span className={cn("font-compact font-semibold tabular-nums", tone === "up" ? "text-market-up" : "text-market-down")}>{formatSigned(item.impact_value, 2)}</span>
          </div>
          <div className="relative font-compact text-[10px] tabular-nums text-zinc-600">{formatSigned(item.change_percent, 2)}%</div>
        </div>
      ))}
    </div>
  );
    ...
    <div className="space-y-1.5">
      ...
      {rows.slice(0, 6).map((item) => (
        <div key={`${title}-${item.symbol}`} className="flex items-center gap-2 text-xs">
          <span className="w-11 font-compact font-semibold text-zinc-200">{item.symbol}</span>
          <div className={cn("h-1.5 flex-1 overflow-hidden rounded-full bg-background", tone === "down" && "flex justify-end")}>
            <div
              className={cn("h-full", tone === "up" ? "bg-market-up" : "bg-market-down")}
              style={{ width: `${(Math.abs(item.net_val || 0) / maxValue) * 100}%` }}
            />
          </div>
          <span className={cn("w-14 text-right font-compact text-[11px] font-semibold tabular-nums", tone === "up" ? "text-market-up" : "text-market-down")}>
...
function Metric({ label, value, className }: { label: string; value: number; className: string }) {
  return (
    <div className="rounded border border-panel-border bg-background p-2">
      <div className={cn("font-compact text-lg font-semibold tabular-nums", className)}>{value}</div>
...
function EmptyState({ label }: { label: string }) {
  return (
    <div className="flex min-h-24 items-center justify-center rounded border border-dashed border-panel-border bg-background/60 px-4 text-center text-xs font-medium text-zinc-500">
      {label}
    </div>
  );
}
