...
export function TetherGate({ onGateUnlock }: { user: any, onGateUnlock: () => void }) {
  ...
  const handleLink = async () => {
    ...
    try {
      ...
      alert("Could not verify SoulKey. Please try again later.");
    } finally {
      setIsLinking(false);
    }
  };
      ...
      >
        ...
        <div className="relative z-10 text-center">
          ...
          <p className="text-muted-foreground font-medium mb-10 max-w-sm mx-auto">
            Chào mừng bạn đến với hệ sinh thái Brokez. Để mở khóa dữ liệu và chiến lược đầu tư chuyên nghiệp, vui lòng nhập mã <span className="text-white">SoulKey</span> được cung cấp bởi Broker của bạn.
          </p>
          ...
          <div className="space-y-6">
            ...
            <button 
              ...
              className="w-full py-6 bg-primary text-black rounded-3xl font-black uppercase tracking-widest text-xs shadow-xl shadow-primary/30 hover:bg-primary/80 hover:shadow-primary/40 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
            >
