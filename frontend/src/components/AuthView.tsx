      ...
      >
        <div className="relative flex min-h-[520px] flex-col justify-between overflow-hidden border-b border-panel-border bg-zinc-950 p-7 md:border-b-0 md:border-r md:p-10">
          ...
          <div className="relative">
            <div className="mb-12 flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-semibold text-zinc-100">Brokerz</h1>
                <p className="text-xs text-zinc-500">Mirae Asset workflow</p>
              </div>
            </div>
            ...
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="space-y-4"
              >
                <div className="text-xs font-semibold text-primary">{copy.eyebrow}</div>
                <h2 className="max-w-md text-3xl font-semibold leading-tight text-zinc-100 sm:text-4xl">
                  {copy.title}
                </h2>
        ...
        <div className="bg-[#151515] p-7 md:p-10">
            ...
            <div className="relative grid grid-cols-2 gap-1">
              {(["broker", "investor"] as const).map((item) => (
                <button
                  ...
                  className={cn(
                    "relative flex items-center justify-center gap-2 rounded px-3 py-3 text-xs font-semibold transition-colors",
                    mode === item ? "text-primary-foreground" : "text-zinc-400 hover:text-zinc-100"
                  )}
                >
                  {mode === item && (
                    <motion.span
          ...
          <button
            ...
            className="mb-6 flex w-full items-center justify-center gap-3 rounded border border-panel-border bg-zinc-900 px-4 py-3 text-sm font-medium text-zinc-200 transition-colors hover:border-primary/60 hover:text-zinc-100"
          >
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <label className="block space-y-2">
              <span className="ml-1 text-xs font-semibold text-zinc-500">Email</span>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded border border-panel-border bg-background py-3 pl-11 pr-4 text-sm text-zinc-100 outline-none transition-colors focus:border-primary"
                />
              </div>
            </label>

            <label className="block space-y-2">
              <span className="ml-1 text-xs font-semibold text-zinc-500">Mật khẩu</span>
              <div className="relative">
            ...
            {error && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="flex items-center gap-3 rounded border border-market-down/25 bg-market-down/10 p-3 text-xs font-semibold text-market-down"
              >
            ...
            <button
              disabled={loading}
              className="flex w-full items-center justify-center gap-2 rounded bg-primary py-4 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary-hover disabled:cursor-not-allowed disabled:opacity-60"
            >
              {loading ? (
                <div className="h-5 w-5 animate-spin rounded-full border-2 border-primary-foreground/25 border-t-primary-foreground" />
              ) : (
                <>
                  Đăng nhập
    ...
    <div className="relative mt-8">
      ...
      >
        <div className="mb-4 flex items-center justify-between gap-4">
          <div className="text-sm font-semibold text-zinc-100">{copy.accent}</div>
          <div className="rounded border border-panel-border bg-zinc-950 px-2.5 py-1 font-compact text-[10px] font-semibold text-primary">
            Secure access
          </div>
        </div>
