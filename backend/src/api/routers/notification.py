...
from src.models.schema import Notification, Profile
...
async def get_notifications(user_id: str = "", actor: CurrentActor = Depends(get_current_actor), db: Session = Depends(get_db)):
...
async def mark_all_as_read(user_id: str = "", actor: CurrentActor = Depends(get_current_actor), db: Session = Depends(get_db)):
